'''
    ...
    by optimizing the hyperparameters on a small subset of the remaining test session.
'''
...
if __name__ == "__main__":
    ...
    num_model_evaluations = 15 # Number of hyperparameter search runs.
    subject_index = 0 # from [0,1]
    split_index = 0 # selected split index from [0,1,2]
    ...
    ### Create the test/validation split indices. ###
    X = np.linspace(0,test_dataset.__len__()-1, test_dataset.__len__(),dtype='int32') # radargram sequence indices.
    y = test_dataset.y_set_categorical

    # create a stratified set of indices for the training and validation set.
    num_classes = 50
    test_fraction = 0.8
    validation_fraction = 0.2
    test_indices, validation_indices = sklearn.model_selection.train_test_split(X, 
    ...
    expected_counts_per_class_test = test_dataset.__len__()/num_classes*test_fraction
    expected_counts_per_class_valid = test_dataset.__len__()/num_classes*validation_fraction
    ...
    train_indices = np.linspace(0, train_dataset.__len__()-1, train_dataset.__len__(), dtype='int32');
    ...
    validation_data_loader = torch.utils.data.DataLoader(test_dataset, 
                                                    batch_size=hyperparameters['batch_size'], 
                                                    shuffle=False, 
                                                    num_workers=0,
                                                    collate_fn=rs_dataset.pad_seqs_and_collate,
    ...
    for run_index in range(num_model_evaluations):
        ...
        num_hidden_units = (torch.randint(low=20, high=50, size=(1,))).item()
